...
public class OctopusEyesController : MonoBehaviour {
    public GameObject[] eyelids;
    public string eyeMaterialName = "eye";
        ...
        if (eyes.Length > 0) {
            ...
            for (int index = 0; index < eyes.Length; index++) {
                ...
                if (meshRenderer != null) {
                    ...
                    if (eyeMaterialName == null || eyeMaterialName.Length == 0) {
                        // Just use the first one we find if no name set
                        eyeMaterial = meshRenderer.material;
                    }
                    else {
                        // Search through the materials for the correct one

                        for (int materialIndex = 0; materialIndex < meshRenderer.materials.Length; materialIndex++) {
                            Material material = meshRenderer.materials[materialIndex];

                            Debug.Log("Checking material " + material.name + " with " + eyeMaterialName);
                            if (material.name.Equals(eyeMaterialName + " (Instance)")) {
    ...
    private Vector2 DetermineEyeOffsetForPointOfInterest(int index) {
        ...
        Vector3 directionToPointOfInterest =  pointOfInterest.position - eye.transform.position;
        ...
        if (productXZ >= 0.0f) {
            ...
            // NOTE: The shader uses negative for 'up' so negate the dot product in the y-axis to compensate
